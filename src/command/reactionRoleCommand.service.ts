...
export class ReactionRoleCommandService extends CommandServiceBase {
  ...
  public add(role: string, emoji: string, messageUrl: string) {
    const messageUrlService = new MessageUrlService(
      messageUrl,
      this._guild.id,
      this._interaction.client
    );
    messageUrlService
      .getMessage()
  ...
  private _onRemoveSuccess(role: string[] | Model[]) {
    ...
    let embed = this._getEmbedMessage()
      .setTitle("身分組設定已移除")
      .setFields({ name: "身分組", value: fieldValue });
    this._interaction.reply({ embeds: [embed], ephemeral: false });
  }
  ...
  private _onListSuccess(res: any) {
    const resultList: EmbedField[] = res.map(
      (item: { role_id: any; reaction: string; message_url: any }) => {
