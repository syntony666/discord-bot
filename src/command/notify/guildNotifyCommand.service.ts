...
export class GuildNotifyCommandService {
  ...
  public off(type: GuildNotifyOperation) {
    ...
    this._guildDTO
      .update(updateColumn, { where: { guild_id: this._guildId } })
  ...
  private _onGuildJoinSuccess(channel: string, message: string) {
    let embed = this._getEmbedMessage()
      .setTitle("伺服器成員加入通知已設定")
      .setFields(
        { name: "頻道", value: channelMention(channel), inline: true },
        {
          name: "測試訊息",
          value: this._getGuildJoinMessage(message),
          inline: true,
        }
      );
  ...
  private _onGuildLeaveSuccess(channel: string, message: string) {
    let embed = this._getEmbedMessage()
      .setTitle("伺服器成員離開通知已設定")
      .setFields(
        { name: "頻道", value: channelMention(channel), inline: true },
        {
          name: "測試訊息",
          value: this._getGuildLeaveMessage(message),
          inline: true,
        }
      );
    this._interaction.reply({ embeds: [embed], ephemeral: false });
  ...
  private _onMessageDeleteSuccess(channel: string) {
    let embed = this._getEmbedMessage()
      .setTitle("訊息刪除通知已設定")
      .setFields({ name: "頻道", value: channelMention(channel) });
    this._interaction.reply({ embeds: [embed], ephemeral: false });
  }
  ...
  private _onListSuccess(res: any) {
    ...
    let embed = this._getEmbedMessage()
      .setTitle("通知設定")
      .setFields(join, leave, delete_notification);
    this._interaction.reply({ embeds: [embed], ephemeral: false });
  }
  ...
  private _getGuildJoinMessage = (message: string) =>
    message.replace("{m}", userMention(this._interaction.user.id));
  private _getGuildLeaveMessage = (message: string) =>
    message.replace("{m}", this._interaction.user.tag);
}
