...
export class StatusCommandService {
    ...
    private _avatar_bg: AttachmentBuilder = new AttachmentBuilder('./assets/avatar_bg.png');
    ...
    public bot() {
        const embed = this._getEmbedMessage()
            .setTitle(this._interaction.user.username)
            ...
            .then(owner => {
                ...
                const embed = new EmbedBuilder()
                    ...
                    .setFields(
                        ...
                        { name: '創立時間', value: time(this._guild.createdAt), inline: true },
                        { name: '成員數量', value: `${this._guild.memberCount}`, inline: false },
                    )
                    .setThumbnail(this._guild.iconURL() ?? '')
                    .setFooter({ text: this._interaction.user.tag, iconURL: this._interaction.user.avatarURL() ?? '' });
                this._interaction.reply({ embeds: [embed], files: [this._logo], ephemeral: false });
            })
    }
